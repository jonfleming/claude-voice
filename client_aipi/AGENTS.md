...
**Environment:**
- Target board: `esp32:esp32:esp32s3` (ESP32-S3 core, AIPI-Lite)
- Sketch: `client_aipi.ino`
- FQBN: `esp32:esp32:esp32s3:FlashSize=16M,PartitionScheme=fatflash,PSRAM=opi`

### Build firmware
- **Compile for AIPI-Lite (ESP32-S3):**
  ```sh
  arduino-cli compile --fqbn esp32:esp32:esp32s3:FlashSize=16M,PartitionScheme=fatflash,PSRAM=opi
  ```
...
- **Discover connected devices:**
  ...
  ```
- **Upload (specify your port, e.g., /dev/ttyACM0 or COM3):**
  ```sh
  arduino-cli upload -p /dev/ttyACM0 --fqbn esp32:esp32:esp32s3:FlashSize=16M,PartitionScheme=fatflash,PSRAM=opi
  ```
  *NOTE:* Change `/dev/ttyACM0` to your actual port—use `arduino-cli board list` to find connected devices.
